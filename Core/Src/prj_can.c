...
bool prj_can_bus_set_silent(const bool enable) {
	...
	PrjCanBus.handle.Init.Mode = enable ? CAN_MODE_SILENT : CAN_MODE_NORMAL;
    return true;
}
...
void prj_can_bus_enable(void) {
	if (PrjCanBus.bus_state == OFF_BUS) {
		...
		PrjCanBus.handle.Init.Mode = CAN_MODE_NORMAL;
...
void prj_can_bus_run(void) {
	...
	uint32_t status = HAL_CAN_AddTxMessage(&PrjCanBus.handle, &txCanBusTmpBuf.Header, txCanBusTmpBuf.Body, &mailbox_txed);
